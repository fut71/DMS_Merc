...
/// <para>
/// Box2D(Vector2 위치, Vector2 사이즈, int 레이어, Vector4 RGBA, , float 두께, Vector4 테두리 색상, bool 보임 여부, bool 3D깊이감 여부);
/// </para>
/// </summary>
struct Box2D
	: public Component
{
public:
	Box2D(std::shared_ptr<Entity> _owner, Vector2 _position = Vector2(), Vector2 _sizeWH = Vector2(), int _layer = 0, 
	...
	{
		return std::make_shared<Box2D>(*this);
	}
